...
Geheime_Mission = Attacke(name="Geheime Mission", text="90 Schaden. Nicht konterbar", keywords=[], type=0,targets=[True,0,0,0])
...
Konter = Attacke(name="Konter", text="Kontere eine Attacke", keywords=[Schnell, Super3], type=0,targets=[0,0,True,0])
...
Lehren = Attacke(name="Lehren", text="Verleihe einem Charakter die Attacke Schwertschalag", keywords=[], type=0,targets=[True,0,0,0])
...
Letzter_Wille = Attacke(name="Letzter Wille", text="5 Mal 20 Schaden", keywords=[Einmalig], type=0,targets=[True,0,0,0])
...
Schneesturm = Attacke(name="Schneesturm",
                      ...
                      keywords=[Passiv], type=0)
Schnell_Atk = Attacke(name="Schnell", text="Bevor dem Zugende: verleihe einer deiner Attacken bis zum Zugende Schnell", keywords=[Extra, Schnell], type=0,targets=[True,True,0,0])
...
Seelenschlag = Attacke(name="Seelenschlag", text="120 Schaden. Du erleidest 50 Schaden", keywords=[], type=0,targets=[True,0,0,0])
Sichere_und_geheime_Mission = Attacke(name="Sichere und geheime Mission",
                                      ...
                                      type=0,targets=[True,0,0,0])
Sicherer_Schlag = Attacke(name="Sicherer Schlag", text="90 Schaden. Schaden nicht veränderbar", keywords=[], type=0,targets=[True,0,0,0])
...
Zellteilung = Attacke(name="Zellteilung",
                      ...
                      keywords=[], type=0,targets=[True,0,0,0])
Zwei_Wünsche = Attacke(name="Zwei Wünsche", text="Kontere eine Attacke", keywords=[Zweimalig, Schnell], type=0,targets=[0,0,True,0])
...
Zyklus_des_Lebens = Attacke(name="Zyklus des Lebens",
                            ...
                            keywords=[Passiv], type=0)
Über_dem_Horizont = Attacke(name="Über dem Horizont", text="erhalte 120 Leben", keywords=[Einmalig, Schnell], type=0,targets=[True,0,0,0])
...
Wachsames_Auge2 = Attacke(name="Wachsames Auge", text="Dein Gegner verrät dir seine Attacken",
                          keywords=[Einmalig, Extra], type=1)
Wieder_normal = Attacke(name="Wieder normal", text="Zerstöre eine Passivattacke", keywords=[Schnell], type=1,targets=[True,True,0,0])
...
entferne_Schnell_Schaden = Attacke(name='Event - Effekt von "Schneller" abgelaufen'
                            ,text="entferne Schnell und Attacke kann keinen Schaden machen von einer Attacke",type=2)
...
def leben_zahlen(lobby: Lobby, c1: Client, pay1: int, c2: Client, pay2: int):
    if lobby.phase == 1:
        lobby.phase = 2
...
def real_heal_calc(lobby:Lobby, attacke: AttackeEingesetzt, mod: int):
    return max(0, min(mod + lobby.healmod+attacke.owner.stats.heil_wut, attacke.t_1.stats.maxLeben - attacke.t_1.stats.leben))
...
def monster(lobby: Lobby, owner: Spieler | Monster, angriff: int, leben: int, spott: bool = 0):
    lobby.clients[owner.spieler_id].spieler.monster.append(Monster(stats=Stats(leben=leben, maxLeben=leben, spott=spott, atk_eingesetzt=(1,0),attacken=[
        AttackeBesitz(Attacke(name="Schaden", text=str(angriff) + " Schaden", type=2))]), spieler_id=owner.spieler_id))
...
def resurrect(lobby: Lobby, target: Spieler | Monster):
    ...
    if sp.gy:
        ...
        mon.stats = Stats(leben=mon.stats.maxLeben,maxLeben=mon.stats.maxLeben,attacken=mon.stats.attacken,spott=mon.stats.spott,atk_eingesetzt=(1,0))
                    ...
                    match atk.attacke.name:
                        case "Alles oder nichts":
                            if atk.t_1 == atk.owner:
                                ...
                                t_1, t_atk,t_stk,t_2= await ask_targets(lobby.clients[atk.owner.spieler_id].client,l_ask[0],l_ask[1],l_ask[2],l_ask[3])
                                ev = Event(time=(lobby.turntime//10)*10+10,event=AttackeEingesetzt(attacke=dupe.attacke,owner=atk.owner,t_1=t_1,t_atk=t_atk,t_stk=t_stk,t_2=t_2))
                        ...
                        case "Feurige Waffen":
                            ...
                            lobby.events.append(Event((lobby.turntime//5)*5+5,AttackeEingesetzt(entferne_Wut,atk.owner)))
                        case "Finale":
                        ...
                        case "Gedankenkontrolle":
                            if atk.attacke.type == 0:
                                ...
                                t_1, t_atk, _, _ = await ask_targets(client=lobby.clients[atk.owner.spieler_id].client,t_atk=True,t_1=True)
                                #Todo
                            else:
                                damage(lobby, 60, atk)
                        ...
                        case "Gleichheit":
                            lobby.clients[atk.owner.spieler_id - 1].spieler.stats.leben = lobby.clients[atk.owner.spieler_id].spieler.stats.leben
                            lobby.clients[atk.owner.spieler_id - 1].spieler.stats.maxLeben = lobby.clients[atk.owner.spieler_id].spieler.stats.maxLeben
                        ...
                        case "Schnell":
                            ...
                            lobby.events.append(Event(end_time,AttackeEingesetzt(attacke=erhalte_Schnell,owner=atk.owner,
                                                                                 t_atk=atk.t_atk)))
                            lobby.events.append(Event(end_time+1, AttackeEingesetzt(attacke=entferne_Schnell, owner=atk.owner,
                                                                                  t_atk=atk.t_atk)))
                        case "Schneller":
                            ...
                            for ab in sp.stats.attacken:
                                ...
                                if any(k.category == 1 for k in keys):  # SuperN
                                    ab.last_used = (lobby.turntime // 5 - (not is_my_turn(lobby, sp))) * 5 \
                                        if sp.stats.atk_eingesetzt == (0, 0) else (lobby.turntime // 5 + (not is_my_turn(lobby, sp))) * 5
                            verraten(lobby,atk.owner)
                        case "Wand":
                        ...
                        case "Doppelter Spott":
                            for element in [atk.t_1,atk.t_2]:
                                if element:
                                    if element.spieler_id != atk.owner.spieler_id:
                                        element.stats.spott = 1
